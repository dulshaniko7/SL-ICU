                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.invoices.update", [$invoice->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('price') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="price" id="price" value="{{ old('price', $invoice->price) }}" step="0.01" required>
                            @if($errors->has('price'))
                        ...
                        <div class="form-group {{ $errors->has('total_tax') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="total_tax" id="total_tax" value="{{ old('total_tax', $invoice->total_tax) }}" step="0.01">
                        ...
                        <div class="form-group {{ $errors->has('total_price') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="total_price" id="total_price" value="{{ old('total_price', $invoice->total_price) }}" step="0.01" required>
                        ...
                        <div class="form-group {{ $errors->has('customer') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="customer_id" id="customer_id">
                                @foreach($customers as $id => $customer)
                                    <option value="{{ $id }}" {{ (old('customer_id') ? old('customer_id') : $invoice->customer->id ?? '') == $id ? 'selected' : '' }}>{{ $customer }}</option>
                                @endforeach
                            </select>
