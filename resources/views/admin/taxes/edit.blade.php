                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.taxes.update", [$tax->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('tax_name') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="tax_name" id="tax_name" value="{{ old('tax_name', $tax->tax_name) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('tax_percentage') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="tax_percentage" id="tax_percentage" value="{{ old('tax_percentage', $tax->tax_percentage) }}" step="0.01">
                        ...
                        <div class="form-group {{ $errors->has('countries') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="countries[]" id="countries" multiple>
                                @foreach($countries as $id => $country)
                                    <option value="{{ $id }}" {{ (in_array($id, old('countries', [])) || $tax->countries->contains($id)) ? 'selected' : '' }}>{{ $country }}</option>
                                @endforeach
                            </select>
