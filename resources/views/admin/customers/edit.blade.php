                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.customers.update", [$customer->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('customer_name') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="customer_name" id="customer_name" value="{{ old('customer_name', $customer->customer_name) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('email') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="email" name="email" id="email" value="{{ old('email', $customer->email) }}" required>
                            @if($errors->has('email'))
                        ...
                        <div class="form-group {{ $errors->has('remember_token') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="remember_token" id="remember_token" value="{{ old('remember_token', $customer->remember_token) }}">
                        ...
                        <div class="form-group {{ $errors->has('country') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="country_id" id="country_id" required>
                                @foreach($countries as $id => $country)
                                    <option value="{{ $id }}" {{ (old('country_id') ? old('country_id') : $customer->country->id ?? '') == $id ? 'selected' : '' }}>{{ $country }}</option>
                                @endforeach
                            </select>
